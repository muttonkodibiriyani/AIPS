...
3. **Environment variables:** open **Project → Settings → Environment Variables** (scroll past Build & Development). Until `COMMERCE_GATEWAY_URL` is set **or** `SHOWCASE_DEMO_SEARCH=true`, the site shows a yellow **configuration banner** and search returns **`503`** (no demo).
   ...
   **Large offline demo (200k SKUs, stratified across *all* merch slugs — not stored in Git):** `SHOWCASE_DEMO_ROW_LIMIT` defaults to **200000** in [`apps/showcase/vercel.json`](./apps/showcase/vercel.json). Host the master **`catalog.csv`** at an **HTTPS URL** and set **`SHOWCASE_CATALOG_URL`** in Vercel (Production + Preview). Each build **streams** that file to `apps/showcase/data/.catalog-fetched.csv`, then generates `demo-catalog.json` (still too large to commit). Optionally **`SHOWCASE_CATALOG_FETCH_AUTH`** (Authorization header) if the URL is private. To **avoid re-downloading** every build, set **`SHOWCASE_CATALOG_FETCH_IF_MISSING_ONLY`**=`1` (otherwise the default is **refresh every build**).
